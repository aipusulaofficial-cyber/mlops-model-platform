...
Invalid lifecycle transition -> 400; persistence/provider timeout -> bounded retry only for idempotent reads/writes; repeated dependency failure -> circuit open; promotion must never partially succeed.